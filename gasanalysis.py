...
def write_rootgraphlinearity(vectorx, vectory, errorvectory, graphtitle):
	...
	f = TF1("f","pol1", 0.0, 0.15)
	#MyTGraph.Fit(f, "R")
	f2 = TF1("f2", '[0]+[1]*x-pow(x,[2])', 0.005,1.0)
	#MyTGraph.Fit(f2, "R")
	MyTGraph.Draw("AP")
	a = f.GetParameter(1)
	b = f.GetParameter(0)
	f2 = TF1("f2", '[0]*x+[1]', 0.0, 1.0)
	f2.SetParameter(0,a)
	f2.SetParameter(1,b)
	f2.Draw("same")
...
def write_summaryhv(vectorx, vectory, graphtitle, filename):
	...
	if "ARCO28020" in graphtitle and "10" in graphtitle and "100" not in graphtitle:
		MyTGraph.SetMarkerColor(9)
		MyTGraph.SetLineColor(9)
...
def processsaturation(file, file2, filename, gastype):
	...
	if "937" in gastype:
		filtervalues = [0.0, 100.0, 46.0, 22.0, 10.0, 6.9, 4.6, 2.2, 1.0]
...
def processARCO28020(file, file2, filename):
	...
	if "_att10" in filename and "_att100" not in filename:
		hvvalues = [550, 560, 570, 580, 590, 600, 610, 620, 630, 640, 650, 690, 700]
	if "_att100" in filename:
		hvvalues = [610, 610, 620, 630, 640, 660, 670, 680, 690]
