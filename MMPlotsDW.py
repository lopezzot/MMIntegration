...
def createsummaryplots(path=None, folder=None):
    ...
    for counter, graph in enumerate(ordered_graphscurrent):                       #+" "+str(int(orderedsmeanvoltages[counter]))#
    	tools.write_orderedrootdategraph_datadw(graph.rootdates, graph.newvalues, graph.filename+" "+str(int(orderedsmeanvoltages[counter])), "time (s)", graph.filename[0], rootdirectory)
