...
def createsummaryplot_attenuation(folder, path, gifpath):
	...
	for dat_file in glob.iglob(path+'*.dat'):
		...
		graph_atten, graph_linearity, spikeslayer, spikeduration, duration, sectorsvoltage, meanvoltage, sectorscurrent, meancurrent, spikeseconds = createplot(giffile, dat_file, dat_file[len(path):len(dat_file)-4], folder)
